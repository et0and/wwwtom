...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`pages_blocks_arena\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `);
  await db.run(
    sql`CREATE INDEX \`pages_blocks_arena_order_idx\` ON \`pages_blocks_arena\` (\`_order\`);`,
  );
  await db.run(
    sql`CREATE INDEX \`pages_blocks_arena_parent_id_idx\` ON \`pages_blocks_arena\` (\`_parent_id\`);`,
  );
  await db.run(
    sql`CREATE INDEX \`pages_blocks_arena_path_idx\` ON \`pages_blocks_arena\` (\`_path\`);`,
  );
  ...
  await db.run(
    sql`CREATE INDEX \`pages_blocks_cta_path_idx\` ON \`pages_blocks_cta\` (\`_path\`);`,
  );
  ...
  await db.run(sql`CREATE TABLE \`_pages_v_blocks_arena\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `);
  await db.run(
    sql`CREATE INDEX \`_pages_v_blocks_arena_order_idx\` ON \`_pages_v_blocks_arena\` (\`_order\`);`,
  );
  await db.run(
    sql`CREATE INDEX \`_pages_v_blocks_arena_parent_id_idx\` ON \`_pages_v_blocks_arena\` (\`_parent_id\`);`,
  );
  await db.run(
    sql`CREATE INDEX \`_pages_v_blocks_arena_path_idx\` ON \`_pages_v_blocks_arena\` (\`_path\`);`,
  );
  ...
  await db.run(
    sql`CREATE INDEX \`_works_v_version_version_hero_image_idx\` ON \`_works_v\` (\`version_hero_image_id\`);`,
  );
  await db.run(
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`pages_blocks_arena\`;`);
  ...
  await db.run(sql`DROP TABLE \`_pages_v_blocks_arena\`;`);
