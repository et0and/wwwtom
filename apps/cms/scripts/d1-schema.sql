...
CREATE TABLE `pages_blocks_arena` (
  	`_order` integer NOT NULL,
  	`_parent_id` integer NOT NULL,
  	`_path` text NOT NULL,
  	`id` text PRIMARY KEY NOT NULL,
...
CREATE INDEX `pages_blocks_arena_order_idx` ON `pages_blocks_arena` (`_order`);
CREATE INDEX `pages_blocks_arena_parent_id_idx` ON `pages_blocks_arena` (`_parent_id`);
CREATE INDEX `pages_blocks_arena_path_idx` ON `pages_blocks_arena` (`_path`);
...
CREATE TABLE `_pages_v_blocks_arena` (
  	`_order` integer NOT NULL,
  	`_parent_id` integer NOT NULL,
  	`_path` text NOT NULL,
  	`id` integer PRIMARY KEY NOT NULL,
...
CREATE INDEX `_pages_v_blocks_arena_order_idx` ON `_pages_v_blocks_arena` (`_order`);
CREATE INDEX `_pages_v_blocks_arena_parent_id_idx` ON `_pages_v_blocks_arena` (`_parent_id`);
CREATE INDEX `_pages_v_blocks_arena_path_idx` ON `_pages_v_blocks_arena` (`_path`);
...
CREATE INDEX `works_hero_image_idx` ON `works` (`hero_image_id`);
CREATE INDEX `works_meta_meta_image_idx` ON `works` (`meta_image_id`);
CREATE UNIQUE INDEX `works_slug_idx` ON `works` (`slug`);
CREATE INDEX `works_updated_at_idx` ON `works` (`updated_at`);
CREATE INDEX `works_created_at_idx` ON `works` (`created_at`);
CREATE INDEX `works__status_idx` ON `works` (`_status`);
...
CREATE INDEX `_works_v_parent_idx` ON `_works_v` (`parent_id`);
CREATE INDEX `_works_v_version_version_hero_image_idx` ON `_works_v` (`version_hero_image_id`);
CREATE INDEX `_works_v_version_meta_version_meta_image_idx` ON `_works_v` (`version_meta_image_id`);
CREATE INDEX `_works_v_version_version_slug_idx` ON `_works_v` (`version_slug`);
CREATE INDEX `_works_v_version_version_updated_at_idx` ON `_works_v` (`version_updated_at`);
CREATE INDEX `_works_v_version_version_created_at_idx` ON `_works_v` (`version_created_at`);
CREATE INDEX `_works_v_version_version__status_idx` ON `_works_v` (`version__status`);
CREATE INDEX `_works_v_created_at_idx` ON `_works_v` (`created_at`);
CREATE INDEX `_works_v_updated_at_idx` ON `_works_v` (`updated_at`);
CREATE INDEX `_works_v_latest_idx` ON `_works_v` (`latest`);
CREATE INDEX `_works_v_autosave_idx` ON `_works_v` (`autosave`);
CREATE TABLE `media` (
  	`id` integer PRIMARY KEY NOT NULL,
  	`alt` text,
  	`caption` text,
...
CREATE INDEX `payload_locked_documents_rels_works_id_idx` ON `payload_locked_documents_rels` (`works_id`);
...
INSERT INTO payload_migrations (name, batch, updated_at, created_at) VALUES ('20260210_074241_initial', 1, strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), strftime('%Y-%m-%dT%H:%M:%fZ', 'now'));