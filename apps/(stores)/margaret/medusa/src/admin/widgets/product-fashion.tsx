...
import { Container, Heading, Text, Button, Drawer, IconButton } from "@medusajs/ui";
...
import { useCreateColorMutation, useCreateMaterialMutation } from "../hooks/fashion";
      ...
      <Drawer.Content>
        ...
        <Drawer.Body className="p-4">
          <Form
            ...
            formProps={{
              id: `material-${materialId}-add-color-${name.toLowerCase().replace(/[^\w]/g, "-")}`,
            }}
          >
        ...
        <Drawer.Footer>
          ...
          <Button
            type="submit"
            form={`material-${materialId}-add-color-${name.toLowerCase().replace(/[^\w]/g, "-")}`}
...
const ProductFashionWidget = withQueryClient(({ data }: DetailWidgetProps<AdminProduct>) => {
  ...
  return (
    <Container className="divide-y p-0">
      ...
      <div className="text-fg-subtle dark:text-fg-subtle-dark px-6 py-4">
        ...
        ) : productFashion.isSuccess && productFashion.data && !materialsData.length ? (
