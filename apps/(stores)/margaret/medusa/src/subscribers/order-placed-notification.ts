...
const buildVariantOptionValues = (item: {
  ...
  if (item.variant_option_values && Object.keys(item.variant_option_values).length) {
    return Object.entries(item.variant_option_values).reduce<Record<string, string>>(
      (acc, [key, value]) => {
