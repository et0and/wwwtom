...
export default async function indexProductHandler({
  ...
  const meilisearchService = container.resolve("meilisearchService") as ISearchService;
  ...
  if (name === "product.updated") {
    await meilisearchService.replaceDocuments("products", [product], "products");
    logger.info(`The product ${productId} ${product.title} was updated in MeiliSearch`);
    return;
  }

  await meilisearchService.addDocuments("products", [product], "products");
  logger.info(`The product ${productId} ${product.title} was added to MeiliSearch`);
}
