...
const indexProductsStep = createStep(
  ...
  async (input: ProductDTO[], context) => {
    const meilisearchService = context.container.resolve("meilisearchService") as ISearchService;
    const result = await meilisearchService.addDocuments("products", input, "products");
