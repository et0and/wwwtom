...
export async function GET(req: MedusaRequest, res: MedusaResponse): Promise<void> {
  ...
  const parsed = productTypeFieldsMetadataSchema.safeParse(productType.metadata ?? {});
...
export async function POST(req: MedusaRequest, res: MedusaResponse): Promise<void> {
  ...
  const updatedProductType = await productService.updateProductTypes(productTypeId, {
    metadata: {
