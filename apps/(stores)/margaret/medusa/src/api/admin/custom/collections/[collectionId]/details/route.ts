...
export async function GET(req: MedusaRequest, res: MedusaResponse): Promise<void> {
  ...
  const parsed = collectionFieldsMetadataSchema.safeParse(collection.metadata ?? {});
  ...
  res.json({
    ...
    product_page_heading:
      parsed.success && parsed.data.product_page_heading ? parsed.data.product_page_heading : "",
    product_page_image:
      parsed.success && parsed.data.product_page_image ? parsed.data.product_page_image : null,
    ...
    product_page_cta_link:
      parsed.success && parsed.data.product_page_cta_link ? parsed.data.product_page_cta_link : "",
  });
}
...
export async function POST(
  ...
  const updatedCollection = await productService.updateProductCollections(collectionId, {
    metadata: {
      ...collection.metadata,
      ...customFields,
    },
