...
export class MeiliSearchService extends SearchUtils.AbstractSearchService {
  ...
  constructor(container: any, options: MeiliSearchPluginOptions) {
    ...
    if (process.env.NODE_ENV !== "development") {
      if (!options.config?.apiKey) {
        throw Error("MeiliSearch API key is required for production environments.");
      }
    }

    if (!options.config?.host) {
      throw Error("MeiliSearch host is required. Please provide a host in the configuration.");
    }
  ...
  async addDocuments(indexName: string, documents: Record<string, any>[], type: string) {
