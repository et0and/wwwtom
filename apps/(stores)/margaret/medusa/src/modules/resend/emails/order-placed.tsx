...
import { Text, Column, Heading, Img, Row, Section, Link, Hr } from "@react-email/components";
...
export type OrderPlacedEmailProps = {
  order: Pick<
    ...
    "currency_code" | "email" | "shipping_total" | "subtotal" | "total" | "tax_total"
  > & {
    shipping_address:
      | (Pick<
          ...
          | "phone"
        > & {
          country?: Pick<HttpTypes.AdminRegionCountry, "iso_2" | "name" | "display_name">;
        })
      | null;
    billing_address:
      | (Pick<
...
export default function OrderPlacedEmail({ order, ...emailLayoutProps }: OrderPlacedEmailProps) {
  ...
  return (
    <EmailLayout {...emailLayoutProps}>
      <Heading className="text-2xl font-medium mt-0 mb-10">Order confirmation</Heading>
      <Text className="text-md !mb-6">
        We are pleased to confirm that your order has been successfully placed and will be processed
        shortly. Your order number is #100002.
      </Text>
      <Text className="text-md !mb-6">
        You&apos;ll receive another update once your order is shipped. For any questions, feel free
        to contact us at info@sofasociety.com.
      </Text>
      ...
      <Section className="mb-6">
        <Row>
          <Column className="border border-solid p-4 border-grayscale-200 rounded-xs">
            <Text className="text-grayscale-500 !mt-0 !mb-8">Delivery Address</Text>
            ...
            {order.shipping_address.phone && (
              <Text className="m-0 leading-tight">{order.shipping_address.phone}</Text>
            )}
          </Column>
          <Column className="w-8" />
          <Column className="border border-solid p-4 border-grayscale-200 rounded-xs">
            <Text className="text-grayscale-500 !mt-0 !mb-8">Billing Address</Text>
            <Text className="m-0 leading-tight">
            ...
            {order.billing_address.phone && (
              <Text className="m-0 leading-tight">{order.billing_address.phone}</Text>
            )}
          </Column>
        </Row>
      </Section>
        ...
        {order.items.map((item, index) => {
          return (
            <Fragment key={item.id}>
              {index > 0 && <Hr className="border-t border-solid border-grayscale-100 m-0" />}
              <Row className="py-4">
                <Column>
                  {!!item.thumbnail && (
                    <Link href="/">
                ...
                <Column className="w-full pl-8 relative" valign="top">
                  <Text className="text-md !mt-0 !mb-2">{item.product_title}</Text>
                  <Section className="mb-1">
                    {Object.entries(item.variant_option_values).flatMap(([key, value]) =>
                      typeof value === "string" ? (
                        <Row key={key}>
                          <Column className="flex">
                            <Text className="text-grayscale-500 m-0 text-xs">{key}:</Text>
                    ...
                    <Row className="absolute bottom-0">
                      <Column className="flex">
                        <Text className="text-grayscale-500 m-0 text-xs">Quantity:</Text>
                        <Text className="m-0 text-xs ml-2">{item.quantity}</Text>
                      </Column>
                    </Row>
                  </Section>
                </Column>
                <Column valign="bottom">
                  <Text className="m-0 text-md">{formatter.format(item.total)}</Text>
                </Column>
              </Row>
            </Fragment>
          );
        ...
        <Row>
          ...
          <Column className="w-1/2">
            <Section>
              <Row className="mb-2">
                <Column className="flex">
                  <Text className="text-grayscale-500 m-0 text-base">Subtotal</Text>
                  <Text className="m-0 text-base ml-auto">{formatter.format(order.subtotal)}</Text>
                </Column>
              </Row>
              <Row className="mb-6">
                <Column className="flex">
                  <Text className="text-grayscale-500 m-0 text-base">Shipping</Text>
              ...
              <Row>
                <Column className="flex">
                  ...
                  <Text className="m-0 text-md ml-auto">{formatter.format(order.total)}</Text>
                </Column>
              </Row>
              <Row>
                <Column className="flex">
                  <Text className="text-grayscale-500 m-0 text-xs">Including</Text>
