...
export const UiSelectButton: React.FC<ReactAria.ButtonProps & UiSelectButtonOwnProps> = ({
  variant = "outline",
  className,
  ...props
}) => (
...
export const UiSelectIcon: React.FC<Omit<IconProps, "name"> & { name?: IconNames }> = ({
  name = "chevron-down",
  className,
  ...props
}) => (
...
export const UiSelectValue = <T extends object>({
  ...
  <ReactAria.SelectValue {...props} className={twMerge("truncate", className as string)} />
);
...
export const UiSelectDialog: React.FC<ReactAria.DialogProps> = ({ className, ...props }) => (
  <ReactAria.Dialog
    {...props}
    className={twMerge(
