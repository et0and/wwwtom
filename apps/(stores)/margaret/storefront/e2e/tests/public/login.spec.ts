...
test.describe("Login Page functionality", async () => {
  ...
  test("enter different incorrect creds and verify error message", async ({ loginPage }) => {
    await loginPage.accountLink.click();
    await loginPage.container.waitFor({ state: "visible" });
  ...
  test("successful login redirects to account page", async ({ accountOverviewPage, loginPage }) => {
    await loginPage.accountLink.click();
    await loginPage.container.waitFor({ state: "visible" });
    await loginPage.emailInput.fill("test@example.com");
    await loginPage.passwordInput.fill("password");
  ...
  test("logging out works correctly", async ({ page, accountOverviewPage, loginPage }) => {
    await loginPage.accountLink.click();
    await loginPage.container.waitFor({ state: "visible" });
    await loginPage.emailInput.fill("test@example.com");
    await loginPage.passwordInput.fill("password");
