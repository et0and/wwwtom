...
test.describe("Account profile tests", () => {
  test("Profile completed update flow", async ({
    ...
    await test.step("enter in the billing address", async () => {
      await expect(profilePage.savedBillingAddress).toContainText("No billing address");
    ...
    await test.step("profile completion state", async () => {
      ...
      await expect(profilePage.savedBillingAddress).toContainText("123 Fake Street");
      await expect(profilePage.savedBillingAddress).toContainText("11111, Springdale");
