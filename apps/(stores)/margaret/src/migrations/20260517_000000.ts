...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  // Drop the incorrectly-named version gallery table (should be _products_v_version_gallery)
  await db.run(sql`DROP INDEX IF EXISTS \`_products_v_gallery_order_idx\`;`);
  await db.run(sql`DROP INDEX IF EXISTS \`_products_v_gallery_parent_id_idx\`;`);
  await db.run(sql`DROP INDEX IF EXISTS \`_products_v_gallery_image_idx\`;`);
  await db.run(sql`DROP TABLE IF EXISTS \`_products_v_gallery\`;`);

  // Create version blocks table for ContentBlock
  ...
  // Create version blocks table for ImageBlock
  ...
  // Create version blocks table for YouTubeBlock
  ...
  // Create correctly-named version gallery table
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  // Reverse correctly-named version gallery
  await db.run(sql`DROP INDEX IF EXISTS \`_products_v_version_gallery_image_idx\`;`);
  await db.run(sql`DROP INDEX IF EXISTS \`_products_v_version_gallery_parent_id_idx\`;`);
  await db.run(sql`DROP INDEX IF EXISTS \`_products_v_version_gallery_order_idx\`;`);
  await db.run(sql`DROP TABLE IF EXISTS \`_products_v_version_gallery\`;`);

  // Restore old incorrectly-named table
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`_products_v_gallery\` (
		\`_order\` integer NOT NULL,
		\`_parent_id\` integer NOT NULL,
		\`id\` integer PRIMARY KEY NOT NULL,
		\`image_id\` integer,
  ...
  await db.run(
    sql`CREATE INDEX IF NOT EXISTS \`_products_v_gallery_order_idx\` ON \`_products_v_gallery\` (\`_order\`);`,
  );
  await db.run(
    sql`CREATE INDEX IF NOT EXISTS \`_products_v_gallery_parent_id_idx\` ON \`_products_v_gallery\` (\`_parent_id\`);`,
  );
  await db.run(
    sql`CREATE INDEX IF NOT EXISTS \`_products_v_gallery_image_idx\` ON \`_products_v_gallery\` (\`image_id\`);`,
  );
}
