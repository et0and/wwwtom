...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(
    sql`CREATE INDEX IF NOT EXISTS \`_products_v_version_meta_version_meta_image_idx\` ON \`_products_v\` (\`version_meta_image_id\`);`,
  );
  await db.run(
  ...
  await db.run(
    sql`CREATE INDEX IF NOT EXISTS \`_products_v_autosave_idx\` ON \`_products_v\` (\`autosave\`);`,
  );
