...
export function performSearch(params: SearchParams): Effect.Effect<SearchResult | null, Error> {
  "use server";
  ...
  return Effect.gen(function* () {
    ...
    if (!endpoint || !api_key) {
      yield* Effect.sync(() => logger.error("Search service is not properly configured"));
      return yield* Effect.fail(new Error("Search service is not properly configured"));
    }
    ...
    return results;
  }).pipe(Effect.tapError((error) => Effect.sync(() => logger.error("Search error", error))));
}
