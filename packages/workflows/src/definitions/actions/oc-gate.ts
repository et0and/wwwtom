...
export const ocGate = compositeAction("oc-gate", {
  name: "OpenCode gate",
  description: dedent(`
    Decide whether a comment should invoke an agent workflow. A composite
    step cannot appear in \`jobs.<id>.if:\`, so the job runs unconditionally
    and every step that needs the go/no-go reads
    \`steps.<id>.outputs.triggered\`. The gate itself is a tiny bash check —
    no checkout required.
  `),
  inputs: {
    "allowed-users": {
      description: dedent(`
        Comma-separated login(s) allowed to trigger. Empty means anyone who
        wrote a trigger comment may invoke the agent.
      `),
      default: "et0and",
    },
    "require-label": {
    ...
    triggers: {
      description: dedent(`
        Comma-separated trigger prefixes. A comment triggers when it starts
        with one, or contains it after a space (the original /oc and /opencode
        expressions). Workflows for other agents pass their own phrase.
      `),
      default: "/oc,/opencode",
    },
  },
